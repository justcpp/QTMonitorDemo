...
QtMonitor::QtMonitor(QWidget *parent)
	: QMainWindow(parent)
{
	ui = new Ui::QtMonitorClass();
	ui->setupUi(this);
	this->initForm();
	this->initMenu();
	this->show_video_all();
...
bool QtMonitor::eventFilter(QObject *watched, QEvent *event)
{
...
void QtMonitor::initForm()
{
	...
	for (int i = 0; i < videoCount; i++) {
		...
		widget->setPixmap(QPixmap(":/QtMonitor/bg_novideo.png"));
		widget->setScaledContents(true);
		widgets.append(widget);
	}
}

void QtMonitor::initMenu()
{
...
void QtMonitor::play_video_all()
{

}

void QtMonitor::snapshot_video_one()
{

}

void QtMonitor::snapshot_video_all()
{

}

void QtMonitor::show_video_all()
{
	if (videoType == "1_4") {
		change_video_4(0);
	}
...
void QtMonitor::change_video(int index, int flag)
{
	int count = 0;
	int row = 0;
	int column = 0;
...
void QtMonitor::change_video_4(int index)
{
	hide_video_all();
	change_video(index, 2);
}

void QtMonitor::change_video_6(int index)
{
	hide_video_all();
	if (index == 0) {
...
void QtMonitor::change_video_8(int index)
{
	hide_video_all();
	if (index == 0) {
...
void QtMonitor::change_video_9(int index)
{
	hide_video_all();
	change_video(index, 3);
}

void QtMonitor::change_video_16(int index)
{
	hide_video_all();
